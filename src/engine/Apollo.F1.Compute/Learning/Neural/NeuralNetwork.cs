...
public class NeuralNetwork : ICostFunction<NeuralOptimizationContext, NeuralPredictionContext>
{
    ...
    public MatrixStorage FeedForward(MatrixStorage x, NeuralPredictionContext predictionContext, bool computeGradients = false)
    {
        ...
        for (int i = 0; i < length; ++i)
        {
            ...
            {
                compute.PerformOn(preactivation[i]).Into(activation[i]).InsertColumn(1.0); // fully activated layer (added bias)
                last = activation[i];
            }
            else
            {
    ...
    private void Backpropagation(MatrixStorage x, MatrixStorage y, NeuralOptimizationContext optimizationContext, NeuralPredictionContext predictionContext)
    {
        ...
        for (int i = OutputLayerIdx - 1; i >= InputLayerIdx + 1; --i)
        {
            ...
            compute.PerformOn(preactivationGradient[i - 1]).Into(preactivationGradientBiased[i - 1]).InsertColumn(1.0);
    ...
    public double ComputeCost(MatrixStorage x, MatrixStorage y, NeuralPredictionContext predictionContext)
    {
        ...
        compute.PerformOn(y).Into(invertedOutput[yIdx]).MultiplyBy(-1.0);
        compute.PerformOn(h).Into(invertedOutput[hIdx]).MultiplyBy(-1.0);
        compute.PerformOnSelf(h).Log();

        invertedOutput[yIdx].PointwiseMultiply(h, h);
        invertedOutput[yIdx].Add(1.0, invertedOutput[yIdx]);
        invertedOutput[hIdx].Add(1.0, invertedOutput[hIdx]);
